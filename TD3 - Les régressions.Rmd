...
prebla <- db3 %>% 
  arrange(info_age) %>%
  filter(row_number() > 2) %>% 
  mutate(groupe_temps = ntile(info_age, 3),
         temps = factor(if_else(groupe_temps == 1, "T0",
                         if_else(groupe_temps == 2, "T1", "T2")),
                        levels = c("T0", "T1", "T2"))) %>% 
  mutate(groupe = factor(if_else(info_sexe == "Femme", "controle", "experimental"),
                         levels = c("controle", "experimental")),
         intentions = tpb_int,
         attitudes = tpb_att,
         normesSubjectives = tpb_norm,
         controlePercu = tpb_pbc) %>% 
  dplyr::select(temps, groupe, intentions, attitudes, normesSubjectives, controlePercu)
